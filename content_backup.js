...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  console.log("Получено сообщение от background.js:", message);
  if (message.action === "openEmptyChat") {
    // Открываем пустой чат
    openEmptyChat();
  }
});

// Функция для открытия пустого чата
function openEmptyChat() {
  console.log("Открываем пустой чат");
  // Удаляем предыдущий контейнер с результатом, если он существует
  const existingContainer = document.getElementById("chatgpt-result-container");
  if (existingContainer) {
    existingContainer.remove();
  }

  // Создаем новую пустую историю чата
  chatHistory = [];

  // Показываем пустой чат
  showResultOnPage("Cognify Chat", "");
}

// Функция для получения текста всей страницы
function getPageText() {
  // Получаем текст страницы, ограничивая длину
  const fullText = document.body.innerText || "";
  // Ограничиваем длину до ~5000 символов чтобы не перегружать API
  return fullText.slice(0, 5000).trim();
}
...
document.addEventListener("mouseup", function (event) {
  ...
  if (selectedText.length > 0) {
    ...
    // Загружаем сохраненный режим и API-ключ, используя глобальные переменные
    chrome.storage.local.get(
      ["useApi", "apiKey", "useWideContext"],
      function (result) {
        if (result.useApi !== undefined) {
    ...
    function showResultOnPage(title, content) {
      console.log("Показываем результат на странице:", title);
      ...
      if (chatHistory.length === 0 && title !== "Cognify Chat") {
        // Если это не пустой чат, то добавляем выделенный текст как первое сообщение пользователя
        chatHistory = [
          { role: "user", content: window.getSelection().toString().trim() },
        ];
      ...
      const handleSendMessage = async () => {
        ...
        // Scroll chat to see the user's message (with smooth animation)
        ...
        try {
          ...
          const apiMessages = [...chatHistory]; // Копируем историю чата

          // Если это первое сообщение и чат был создан через иконку расширения (пустой чат),
          ...
          // Запрашиваем ответ у API
          let response;
          if (useApi) {
            response = await fetchChatOpenAI(apiMessages);
          } else {
            // Если API не используется, просто открываем ChatGPT с запросом
            const promptText = `Предыдущий контекст: ${chatHistory
              .slice(0, -1)
          ...
          // Scroll to the user's last message
          const userMessages = chatContent.querySelectorAll(".user-message");
          if (userMessages.length > 0) {
            const lastUserMessage = userMessages[userMessages.length - 1];
            lastUserMessage.scrollIntoView({
      ...
      // Don't scroll initially, let the user see the beginning of the conversation
      if (chatHistory.length > 0) {
        // If we have messages, scroll to the first one to ensure it's visible
        const firstMessage = chatContent.querySelector(".chat-message");
        if (firstMessage) {
          setTimeout(() => {
              ...
              if (lastUserMessage) {
                ...
                fetchChatOpenAI(apiMessages)
                  .then((response) => {
                    ...
                    // Scroll to the user's last message
                    const userMessages =
                      chatContent.querySelectorAll(".user-message");
                    if (userMessages.length > 0) {
    ...
    async function handleExplain(e) {
      ...
      if (useWideContext) {
        ...
        const pageText = getPageText();
        promptText = `Объясни "${selectedText}" в контексте этой страницы: "${pageText}"`;
      } else if (context && word.split(/\s+/).length === 1) {
        // If it's one word and there's context
        promptText = `Объясни "${word}" в следующем контексте: "${context}"`;
      } else {
        // If it's a phrase or there's no context
        promptText = `Объясни это: ${selectedText}`;
      }
      ...
      if (useApi) {
        ...
        try {
          ...
          // Scroll to the user's last message before the assistant's response
          const userMessages = chatContent.querySelectorAll(".user-message");
          if (userMessages.length > 0) {
            const lastUserMessage = userMessages[userMessages.length - 1];
            lastUserMessage.scrollIntoView({
          ...
          // Scroll to the user's last message before the error message
          const userMessages = chatContent.querySelectorAll(".user-message");
          if (userMessages.length > 0) {
            const lastUserMessage = userMessages[userMessages.length - 1];
            lastUserMessage.scrollIntoView({
    ...
    async function handleTranslate(e) {
      ...
      if (useWideContext) {
        // If wide context mode is enabled, add entire page text
        const pageText = getPageText();
        promptText = `Переведи "${selectedText}" в контексте этой страницы: "${pageText}"`;
      } else if (context && word.split(/\s+/).length === 1) {
        // If it's one word and there's context
        promptText = `Переведи "${word}" в следующем контексте: "${context}" и приведи другие значения этого слова (пронумеруй, с каждым значением в отдельном абзаце)`;
      } else {
        // If it's a phrase or there's no context
        promptText = `Переведи: "${selectedText}"`;
      }
